...
def solve_baseline(pos: Tuple[float, float], vx0: float, t_steps: int = 200,
                   tf_guess: float = 20.0, y0_guess: float = 1.0,
                   alpha: float = 10.0, beta: float = 25.0, gamma: float = 3.0,
                   nu: float = 0.0, G: float = 2.0):
    ...
    def bc(ya, yb, p):
        ...
        return np.array([
            ...
            yb[6] - 2 * beta * yb[2],
            yb[7] - 2 * beta * yb[3],
            yb[4] * yb[2] + yb[5] * yb[3] + yb[6] * uxf + yb[7] * (uyf - G)
            - alpha * (uxf ** 2 + uyf ** 2) - gamma + nu * (np.minimum(0, yb[1])),
        ])
...
def solve_with_final_angle(
    pos: Tuple[float, float], vx0: float, t_steps: int = 200,
    tf_guess: float = 20.0, y0_guess: float = 1.0,
    alpha: float = 10.0, beta: float = 25.0, gamma: float = 3.0,
    nu: float = 0.0, G: float = 2.0, rho: float = 0.01, final_angle_on: bool = True,
):
