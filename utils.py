...
def make_trajectory_plot(
    x, y, obstacles=None, xlim=None, guess=None, title=None, save_file=None
):
    plt.style.use("dark_background")
...
def lunar_lander_final_angle_v2(
    pos_init,
    v_init,
    tf_guess=20,
    y0_guess=1,
    ...
    def ode(t, y, p):
        ...
        return tf * np.array(
            [
                ...
                np.zeros_like(y[0]),
                nu * (1 - np.heaviside(y[1], 0))
                + final_angle_on * zeta * np.heaviside(-y[1] + eps, 0),
                -y[4],
                -y[5],
            ]
        )
    ...
    def bc(ya, yb, p):
        ...
        return np.array(
            [
                ...
                + yb[5] * yb[3]
                + yb[6] * uxf
                + yb[7] * (np.maximum(0, uyf) - G)
                - alpha * (uxf**2 + uyf**2)
                - gamma
                + nu * (np.minimum(0, yb[1]))
                - final_angle_on * zeta * np.minimum(0, yb[1] - eps) * uxf**2,
            ]
        )
...
try:
    ...
    def lunar_lander_final_angle(
        ...
        return _solve_with_final_angle(
            pos_init,
            v_init,
            t_steps=t_steps,
            tf_guess=tf_guess,
    ...
    def make_trajectory_plot(
        x, y, obstacles=None, xlim=None, guess=None, save_file=None
    ):
        return _plot_trajectory(
            x, y, obstacles=obstacles, xlim=xlim, guess=guess, save_file=save_file
        )
